...
for j, target in enumerate(tqdm(group_list)):
    ...
    pep_total_list = df[~df['allele'].isin(target)]['Peptide seq'].unique()
    #pep_total_list = df[df['allele'].str.contains(allele)]['Peptide seq'].unique()
    pep_list.sort()
    pep_total_list.sort()
    ...
    for t_df in result_df:
        ...
        if len(t_df['group'].unique()) > 1:
            total_df.append(t_df)
    try:
        pd.concat(total_df).drop_duplicates().to_csv(f'{allele} group{j + 1} in total group.csv')
